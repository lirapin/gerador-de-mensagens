...
// Inicializar a lista de incidentes
...
function atualizarListaIncidentes() {
  ...
  const incidentes = Object.keys(incidentesAtivos);
  ...
  incidentes.forEach(id => {
    const incidente = incidentesAtivos[id];
    const item = document.createElement('div');
    item.className = `incident-item ${incidenteAtual === id ? 'active' : ''}`;
    item.innerHTML = `
      <div>
        <div class="incident-id">${id}</div>
        <div class="incident-info">${incidente.tipo} - ${incidente.incidente || incidente.incidenteManobra}</div>
      </div>
      <button class="delete-incident" onclick="excluirIncidente('${id}')"><i class="fas fa-trash"></i></button>
    `;
    item.addEventListener('click', () => carregarIncidentePorId(id));
...
function salvarIncidente() {
  ...
  if (!incidenteId) {
    incidenteId = prompt('Digite um ID para este incidente:');
  ...
  incidentesAtivos[incidenteId] = {
    tipo: tipoMensagem,
    ...dados,
    dataCriacao: new Date().toISOString()
  };

  localStorage.setItem('incidentesAtivos', JSON.stringify(incidentesAtivos));
  incidenteAtual = incidenteId;
  atualizarListaIncidentes();
  
  alert(`Incidente ${incidenteId} salvo com sucesso!`);
}
...
function carregarIncidente() {
  const incidenteId = document.getElementById('incidenteId').value.trim();
  if (!incidenteId) {
    alert('Por favor, digite um ID de incidente');
    return;
  }

  carregarIncidentePorId(incidenteId);
}

function carregarIncidentePorId(incidenteId) {
  if (!incidentesAtivos[incidenteId]) {
    alert('Incidente não encontrado');
    return;
  }

  const incidente = incidentesAtivos[incidenteId];
  document.getElementById('incidenteId').value = incidenteId;
  
  // Selecionar o tipo de mensagem correto
  document.getElementById('tipoMensagem').value = incidente.tipo;
  mostrarFormulario();

  // Preencher os campos do formulário
  if (incidente.tipo === 'rompimento') {
    document.getElementById('topologia').value = incidente.topologia || '';
    document.getElementById('incidente').value = incidente.incidente || '';
    document.getElementById('cidade').value = incidente.cidade || '';
    document.getElementById('distrito').value = incidente.distrito || '';
    document.getElementById('impacto').value = incidente.impacto || '';
    document.getElementById('base').value = incidente.base || '';
    document.getElementById('abertura').value = incidente.abertura || '';
    document.getElementById('previsao').value = incidente.previsao || '';
    document.getElementById('acionamento').value = incidente.acionamento || '';
  } else if (incidente.tipo === 'manobra') {
    document.getElementById('topologiaManobra').value = incidente.topologiaManobra || '';
    document.getElementById('ticketManobra').value = incidente.ticketManobra || '';
    document.getElementById('incidenteManobra').value = incidente.incidenteManobra || '';
    document.getElementById('cidadeManobra').value = incidente.cidadeManobra || '';
    document.getElementById('distritoManobra').value = incidente.distritoManobra || '';
    document.getElementById('impactoManobra').value = incidente.impactoManobra || '';
    document.getElementById('baseManobra').value = incidente.baseManobra || '';
    document.getElementById('responsavelManobra').value = incidente.responsavelManobra || '';
    document.getElementById('executorManobra').value = incidente.executorManobra || '';
    document.getElementById('enderecoManobra').value = incidente.enderecoManobra || '';
    document.getElementById('propostoManobra').value = incidente.propostoManobra || '';
  }

  incidenteAtual = incidenteId;
  atualizarListaIncidentes();
  alert(`Incidente ${incidenteId} carregado com sucesso!`);
}

function excluirIncidente(incidenteId) {
  if (confirm(`Tem certeza que deseja excluir o incidente ${incidenteId}?`)) {
    delete incidentesAtivos[incidenteId];
    localStorage.setItem('incidentesAtivos', JSON.stringify(incidentesAtivos));
    ...
    atualizarListaIncidentes();
    alert(`Incidente ${incidenteId} excluído com sucesso!`);
  }
}

function limparFormularios() {
  // Limpar formulário de rompimento
  ...
  if (tipo === 'inicial') {
    statusCampos.innerHTML = `
      <div class="form-group">
        <label for="informacoesIniciais">Informações do Status Inicial:</label>
        <textarea id="informacoesIniciais" rows="3"></textarea>
      </div>
    `;
    ...
    statusCampos.innerHTML = `
      <div class="form-group">
        ...
        <input type="text" id="statusAtual">
      </div>
      <div class="form-group">
        <label for="validado">Percentual Validado (%):</label>
    ...
    statusCampos.innerHTML = `
      ...
      <div class="form-group">
        <label for="fca">FCA (Fato, Causa, Ação):</label>
        <textarea id="fca" rows="3"></textarea>
      </div>
      <div class="form-group">
...
function atualizarCamposStatusManobra() {
  ...
  if (tipo === 'inicial') {
    statusCampos.innerHTML = `
      <div class="form-group">
        <label for="informacoesIniciaisManobra">Informações do Status Inicial:</label>
        <textarea id="informacoesIniciaisManobra" rows="3"></textarea>
      </div>
    `;
  } else if (tipo === 'atualizacao') {
    statusCampos.innerHTML = `
      <div class="form-group">
        <label for="statusAtualManobra">Status Atual:</label>
        <input type="text" id="statusAtualManobra">
      </div>
      <div class="form-group">
        <label for="validadoManobra">Percentual Validado (%):</label>
    ...
    statusCampos.innerHTML = `
      ...
      <div class="form-group">
        <label for="fcaManobra">FCA (Fato, Causa, Ação):</label>
        <textarea id="fcaManobra" rows="3"></textarea>
      </div>
    `;
  }
}

function validarFormatoDataHora(dataHora) {
...
function verificarAlertaImpacto(topologia, valorImpacto) {
  ...
  if (topologia === 'HFC' && numero >= 10) {
    return ' ⚠️';
  } else if (topologia === 'GPON' && numero >= 300) {
    return ' ⚠️';
  }
...
function gerarMensagem() {
  // Verificar se a topologia foi selecionada
  ...
  // Verificar se o tipo de status foi selecionado
  ...
  camposDataHora.forEach(campo => {
    ...
    if (valor && !validarFormatoDataHora(valor)) {
  ...
  const alertaImpacto = verificarAlertaImpacto(topologia, impactoValor);
  ...
  if (tipoStatus === 'inicial') {
    msg += `## INFORMAÇÕES INICIAIS: ${get('informacoesIniciais')}\n`;
  } else if (tipoStatus === 'atualizacao') {
    msg += `## STATUS ATUAL: ${get('statusAtual')}\n`;
    msg += `## PERCENTUAL VALIDADO: ${get('validado')}%\n`;
  } else if (tipoStatus === 'encerramento') {
    msg += `## DATA E HORA DE ENCERRAMENTO: ${get('encerramento')}\n`;
    msg += `## FCA (FATO, CAUSA, AÇÃO): ${get('fca')}\n`;
